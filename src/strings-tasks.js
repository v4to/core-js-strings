...
function removeFirstOccurrences(str, value) {
  ...
  if (indexOfFirstOccurrence !== -1) {
    return (
      ...
      str.substring(indexOfFirstOccurrence + value.length, str.length)
    );
  }
...
function removeLastOccurrences(str, value) {
  ...
  if (indexOfLastOccurrence !== -1) {
    return (
      ...
      str.substring(indexOfLastOccurrence + value.length, str.length)
    );
  }
...
function isPalindrome(str) {
  const numberOfIterations = Math.floor(str.length / 2);
  const lowerCasedStr = str.toLowerCase().replaceAll(/\W/g, '');

  for (let i = 0; i < numberOfIterations; i += 1) {
    if (
      lowerCasedStr.charAt(i) !==
      lowerCasedStr.charAt(lowerCasedStr.length - 1 - i)
    ) {
      return false;
    }
  }
...
function invertCase(str) {
  let invertCasedStr = '';
  const lowerCasedACode = 'a'.codePointAt(0);
  const upperCasedACode = 'A'.codePointAt(0);
  const lowerCasedZCode = 'z'.codePointAt(0);
  const upperCasedZCode = 'Z'.codePointAt(0);

  for (let i = 0; i < str.length; i += 1) {
    const currentCharacterCode = str.charCodeAt(i);

    if (
      currentCharacterCode >= lowerCasedACode &&
      currentCharacterCode <= lowerCasedZCode
    ) {
      invertCasedStr += str.charAt(i).toUpperCase();
    } else if (
      currentCharacterCode >= upperCasedACode &&
      currentCharacterCode <= upperCasedZCode
    ) {
      invertCasedStr += str.charAt(i).toLowerCase();
    } else {
      invertCasedStr += str.charAt(i);
    }
  }

  return invertCasedStr;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      ...
      if (currentCharacterCode >= lowerCasedACode + 13) {
        ROT13String += String.fromCodePoint(currentCharacterCode - 13);
      } else {
        ROT13String += String.fromCodePoint(currentCharacterCode + 13);
      }
    } else if (
      ...
      if (currentCharacterCode >= upperCasedACode + 13) {
        ROT13String += String.fromCodePoint(currentCharacterCode - 13);
      } else {
        ROT13String += String.fromCodePoint(currentCharacterCode + 13);
      }
    } else {
      ROT13String += str.charAt(i);
    }
  }
